...
#define MESSAGE__TYPE_IMU__YAW 'Y'
...
void ESP__processIncomingChar(const char inByte) {
  if (inByte == 'P') {
    ...
    switch (inByte) {
      ...
      case MESSAGE__TYPE_IMU__YAW:
        _message__started = true;
        _message__type = MESSAGE__TYPE_IMU__YAW;
        _message__type_length = 0;
        _yaw_str = "";
        break;
    }
    ...
    } */else if (_message__type == MESSAGE__TYPE_IMU__PITCH || _message__type == MESSAGE__TYPE_IMU__ROLL || _message__type == MESSAGE__TYPE_IMU__YAW) {
      if (inByte == ' ') {
        _message__type_length += 1;
        ...
        if (inByte == '$') {
          if (_message__type == MESSAGE__TYPE_IMU__PITCH) {
            char __pitch[50];
            _pitch_str.toCharArray(__pitch, 50);
            _pitch = atof(__pitch);
          ...
          } else if (_message__type == MESSAGE__TYPE_IMU__ROLL) {
            char __roll[50];
            _roll_str.toCharArray(__roll, 50);
            _roll = atof(__roll);
            ...
            //  }
            //}
          } else if (_message__type == MESSAGE__TYPE_IMU__YAW) {
            char __yaw[50];
            _yaw_str.toCharArray(__yaw, 50);
            _yaw = atof(__yaw);
          }
...
bool _run() {
  ...
  if (!result) {
    ...
    if (RUN_MODE__is(RUN_MODE_TESTING)) {
      ...
      float axis2 = (float) defaultPpmValues[2];
      ...
      if (_yaw >= 0) {
        axis2 -= (((float) 500) * ((_yaw / ((float) 90)))) * multiplier; // backward
      } else {
        axis2 += (((float) 500) * ((abs(_yaw) / ((float) 90)))) * multiplier; // forward
      }
