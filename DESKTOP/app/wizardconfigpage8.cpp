...
WizardConfigPage8::WizardConfigPage8(QWidget *parent) : QWizardPage(parent) {
    ...
    for (int i = 1, row = 0; i <= 8; i += 2, row += 1) {
        ...
        for (int j = i, k = 0; j <= i + 1; j += 1, k += 1) {
            ...
            QLineEdit * value = new QLineEdit(QString("disarmingValueChannel" + QString(i + k)));
            value->setText(QString::number((int) 1100));
            value->setValidator(new QIntValidator(0, 2000, _layout));

            QLineEdit * time = new QLineEdit(QString("disarmingValueTimeChannel" + QString(i + k)));
